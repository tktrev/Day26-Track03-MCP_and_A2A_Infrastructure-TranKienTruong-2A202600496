    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, personal data'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
def privacy_agent(state: LegalState) -> dict:
    ...
    """
    import asyncio
    result = asyncio.run(llm.ainvoke([SystemMessage(content=prompt)]))
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a specialist in data privacy law, GDPR, CCPA, and personal data protection. "
        "Provide analysis on privacy implications and regulatory requirements. "
        "Keep your response under 200 words."
    )

    llm = get_llm()
    search_tool = __import__('langchain_core.tools', fromlist=['tool']).tool

    @__import__('langchain_core.tools', fromlist=['tool']).tool
    def search_privacy_law(query: str) -> str:
        """Search privacy law knowledge base."""
        knowledge = [
            (["gdpr", "privacy", "data", "personal"], "GDPR (EU) 2016/679: fines up to 4% of global revenue or EUR 20M. Rights: access, rectification, erasure, portability. Lawful basis required for processing."),
            (["ccpa", "california", "consumer"], "CCPA/CPRA: fines $7,500 per intentional violation. Consumer rights: know, delete, opt-out. Business purpose disclosure required."),
            (["data breach", "notification"], "Data breach notification: GDPR 72-hour requirement, state laws vary (US). Individual right of action under CCPA ($100-$750 per consumer)."),
        ]
        query_lower = query.lower()
        results = []
        for keywords, text in knowledge:
